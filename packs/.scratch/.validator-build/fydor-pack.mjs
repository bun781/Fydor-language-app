import { z } from "zod";
import { parseStrictJson } from "./safeJson.mjs";
...
export function parseFydorPack(source) {
    let parsed;
    try {
...
export function countSentences(lessons) {
    return lessons.reduce((total, lesson) => total + lesson.sentences.length, 0);
}
export function estimatePackSize(pack) {
...
export function slugifyPackTitle(title) {
    return title
        .toLowerCase()
        .trim()
        .replace(/[^a-z0-9]+/g, "-")
...
function validateLessonContents(lesson) {
    const errors = [];
...
function containsSurface(sentenceText, surface) {
...
function getRawTitle(value, fallback) {
    if (typeof value !== "object" || value === null || !("title" in value))
        return fallback;
    const title = value.title;
    return typeof title === "string" && title.trim() ? title : fallback;
}
function stablePackId(title, authorName, createdAt) {
