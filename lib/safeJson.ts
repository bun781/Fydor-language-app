...
export function parseStrictJson(source: string, options: { maxBytes?: number; maxDepth?: number } = {}): unknown {
  ...
  if (new TextEncoder().encode(source).byteLength > maxBytes) throw new Error(`JSON exceeds the ${maxBytes} byte limit.`);
  scan(source, maxDepth);
  return JSON.parse(source) as unknown;
}

function scan(source: string, maxDepth: number) {
  let index = 0;
  ...
  function readValue(depth: number) {
    ...
    whitespace();
    const char = source[index];
    if (char === "{") return readObject(depth + 1);
    if (char === "[") return readArray(depth + 1);
  ...
  function readObject(depth: number) {
    ...
    while (true) {
      ...
      keys.add(key); whitespace(); if (source[index++] !== ":") throw new Error("Malformed JSON object.");
      readValue(depth); whitespace(); if (source[index] === "}") { index += 1; return; }
      if (source[index++] !== ",") throw new Error("Malformed JSON object.");
    }
  }
  function readArray(depth: number) {
    index += 1; whitespace(); if (source[index] === "]") { index += 1; return; }
    while (true) { readValue(depth); whitespace(); if (source[index] === "]") { index += 1; return; } if (source[index++] !== ",") throw new Error("Malformed JSON array."); }
  }
  readValue(0); whitespace(); if (index !== source.length) throw new Error("Mixed content is not valid JSON.");
}
